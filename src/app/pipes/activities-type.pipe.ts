...
export class ActivitiesTypePipe implements PipeTransform {
  ...
  transform(activities: IActivityApplication[], activityType: string | undefined): IActivityApplication[] {

    if(activityType === "liked") {
      console.log("ES LIKE!!!")
      const likedArr = activities.filter((activity) => activity.actividad_liked);
    ...
    if(activityType === "removed") {
      const removedArr = activities.filter((activity) => activity.actividad_removed);
